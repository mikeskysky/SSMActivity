		  	...
		  	<td>
		  		<form action="${pageContext.request.contextPath}/workflow/submitTask" method="POST">
			  		<div align="left" class="STYLE21">
				 		...
				 		请假人:<input type="text" name="user.id" value="${leaveBill.user.name}"  disabled="true" style="width: 200px;" /><br/>
				 		请假天数:<input type="text" name="days" disabled="true" style="width: 200px;" value="${leaveBill.days}" /><br/>
				 		请假原因:<input type="text" name="content" disabled="true" style="width: 800px;" value="${leaveBill.content}" /><br/>
				 		请假备注:<textarea rows="2" cols="30" name="remark">${leaveBill.remark}</textarea><br/>
				 		批&emsp;&emsp;注:<textarea rows="5" cols="50" name="comment"></textarea><br/>
				 		<!-- 使用连线的名称作为按钮 -->
				 		<c:if test="${outcomList!=null}">
				 			<c:forEach items="${outcomList}" var="outcome">
